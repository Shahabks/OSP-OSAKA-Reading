...
# use object ID
   ...
   soundid = Remove noise...  0 0 0.025 80 8000 40 Spectral subtraction
	...
	if nuofwrds<3 
		...
		#WARNING
			...
			if speakingtot<polish 
				warning2$ = "Background noise or your speech is limited in content with long pause; it might affect the accuracy of your speech assessment" 
					else 
					warning2$ = " "	
			endif
		...
		if 	ins=4 
			...
			if  speakingrate>4.53 
				   q=4    
					 elsif speakingrate<=4.53 and speakingrate>3.94 
					   q=3.5
				elsif speakingrate<=3.94 and speakingrate>3.35 
					q=3
						 elsif speakingrate<=3.35 and speakingrate>2.93  
							 q=2    
							   elsif speakingrate<=2.93 and speakingrate>=2.5 
								 q=1.16         
								   else 
									 q=1        
				endif
		...
		if articulationrate>=5.12 and balance>=0.8
				qr = 4
					elsif articulationrate>=5.12 and balance<0.8
						qr = 3.8	
							elsif articulationrate<5.12 and articulationrate>=4.65 and balance>=0.8
								qr = 3.6
									elsif articulationrate<5.12 and articulationrate>=4.65 and balance<0.8
										qr = 3.4
		...
		# summarize SCORE in Info window
		   totalscore =(l*2+z*4+o*1+qr*5+w*4+r*4)/20
		...
		if speakingtot<=7 and npausez<=1  
			xx=xx
			elsif speakingtot<=7 and npausez>1
			xx=xx-5
			elsif speakingtot>7 and speakingtot<=20 and npausez>1 and npausez<=4
			xx=xx
			else
			xx=xx-5
		endif
		...
		Erase all
		appendInfoLine: warning0$, newline$
		 appendInfoLine: warning1$, newline$ 
		 appendInfoLine: warning2$, newline$
		 appendInfoLine: warning3$, newline$	 
		 appendInfoLine: appendInfoLine: 'rstd:1',tab$, 'ins:1',tab$, 'qstd:1',tab$, 'lstd:1',tab$, 'nuofwrds:0',tab$,'npause:1',tab$, 'originaldur:0',tab$,'xx:1',tab$, xxban$, tab$, 'avelongpause:0',tab$,'npausez:2'
	endif	
	if nuofwrds<3
		appendInfoLine: warningfun1$
	endif
endif
if int<2
	appendInfoLine: warningfun$
endif